...
namespace Edutor.Data.SqlServer.QueryProcessors
{
    public class GetEventsQueryProcessor : IGetEventsQueryProcessor
    {
        ...
        public QueryResult<Event> GetEventsForSchoolUser(int schoolUser, int groupId, PagedDataRequest requestInfo)
        {
            var q = _session.QueryOver<Event>().Where(nn => nn.Group.GroupId == groupId).OrderBy(nn => nn.CreationDate).Asc.Where(not => not.SchoolUser.UserId == schoolUser);
